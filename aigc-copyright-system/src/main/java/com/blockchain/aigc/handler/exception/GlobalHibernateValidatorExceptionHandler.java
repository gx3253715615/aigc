package com.blockchain.aigc.handler.exception;
...
public class GlobalHibernateValidatorExceptionHandler {
    ...
    public ApiResponse<Void> handleBindException(HttpServletRequest request, BindException ex) {
    ...
    public ApiResponse<Void> handleConstraintViolationException(HttpServletRequest request, Exception ex) {
        ...
        // 处理 HttpMessageNotReadableException (枚举类转换错误)
        if (ex instanceof HttpMessageNotReadableException) {
            ...
            if (cause instanceof com.fasterxml.jackson.databind.exc.InvalidFormatException) {
                com.fasterxml.jackson.databind.exc.InvalidFormatException formatEx =
                        (com.fasterxml.jackson.databind.exc.InvalidFormatException) cause;
                // 判断是否是枚举类转换错误
                Class<?> targetType = formatEx.getTargetType();
                if (targetType.isEnum()) {
                    String errorMsg = String.format("参数格式错误：无效的枚举值 '%s' 对应类型 '%s'",
                            formatEx.getValue(), targetType.getSimpleName());
        ...
        if (ex instanceof ConstraintViolationException) {
            ...
            String errorMsg = constraintEx.getConstraintViolations().stream()
